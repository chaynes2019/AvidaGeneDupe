...
In a first set of experiments, we investigated whether gene duplication facilitated \textit{de novo} evolution of adaptive phenotypic traits.
Replicates with duplication indeed evolved more adaptive traits than under baseline conditions (two-tailed Mann-Whitney U test, $W = 562.5$, Bonferroni-adjusted $p << 0.0001$; Figure \ref{fig:results_panels:slip_duplication}).
In many cases, gene duplication grew genome size substantially; some swelling up to 10-fold beyond their original 100-instruction starting point (Figure \ref{fig:num-coding-sites:total}).
To test whether increased genome size alone could account for gene duplication's adaptive benefit, we performed additional experiments incorporating a ``long-genome'' control treatment with 1,000-site genomes.

\input{fig/adaptive-evolution-rate.tex}

This long-genome control also evolved adaptive traits more readily than baseline conditions (Supplementary Figure \ref{fig:adaptive-evolution-rate-agg}).
However, disaggregating by task complexity reveals genome size to primarily boost simple adaptation.
...
Having observed that slip-duplicate mutations can accelerate adaptive evolution in excess of effects expected from increased genome length alone, we next sought to test whether adaptive outcomes stemmed from genetic information propagated in duplications.
