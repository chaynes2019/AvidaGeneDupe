...
Experiments reported here extend an earlier conference publication \citep{lalejini2017gene}.
...
Logic tasks were also described in terms of minimum required NAND operations as a metric of complexity, as given in Figure \ref{fig:slip_mut_variants} \citep{Lenski2003Evolutionary}.

Except where otherwise noted, experiments were seeded with a 100-instruction ancestral self-replicator.
In all cases, mutations reducing genome size below 100 instructions were disallowed to ensure sufficient genetic material to encode phenotypic traits.
Run length was 200,000 updates across all experiments, where a single update processes on average 30 genome instructions per organism.
This duration was sufficient to observe at least 600 generations of evolution in all trials.
We configured Avida's scheduler to ensure long genomes incurred no inherent disadvantage \citep{Ofria:2009avida}.
...
Genomes are linear sequences of instructions, each encoding one of 27 available operations for basic computations, execution flow control, input and output, and self-replication.
The instruction set is Turing complete and valid in any possible ordering.
...
In our experiments, each could occur with a probability of 0.05 per offspring \citep{Lenski2003Evolutionary}.
...
1) \textit{slip-scramble}, where duplicated code was shuffled to test effects of sequence order,
2) \textit{slip-random}, where duplicated code was replaced with random instructions to test effects of sequence content,
3) \textit{slip-NOP}, where duplicated code was replaced with neutral (``no-operation'') instructions to test the effect of neutral increase in genome size, and
4) \textit{slip-scatter}, where duplicated code was dispersed across the genome to test the effect of insertion locality (Figure \ref{fig:slip_mut_variants}).
...
The first phase tested aggregate differences in evolutionary outcomes between surveyed slip duplication operator variants across runs, while the second focused more heavily on teasing apart evolutionary history and genetic structure within replicates.
For this reason, first-phase experiments prioritized higher replicate counts, while second-phase experiments prioritized more detailed lineage tracking of individual genomic sites.
We conducted 100 evolutionary replicates per treatment for first-phase experiments, and 30 replicates for second-phase experiments.
...
Adaption outcomes shown in Figure \ref{fig:results_panels} were also derived from second-phase experiments.
...
In a postprocessing step, we applied Avida's ``analyze mode'' to assess phenotypic traits at each step in the lineage leading to the final dominant genotype.

For investigations involving the evolutionary history of individual genomic sites, we used mutational metadata saved with lineage files to identify corresponding sites between parent and offspring.
