...
We also exploit the opportunity to trace duplicated sites across exact lineage histories \citep{dolson2024phylotrack}, and thereby directly ascertain their role in later innovation.

\input{fig/slip_mut_variants.tex}

We conducted digital experiments using Avida, software which administers populations of self-replicating computer programs --- providing, essentially, a digital ``model organism'' \citep{Ofria:2009avida}.
...
These adaptive sites were identified through single-site knockout screens \citep{moreno2024cryptic}, disabling genome instructions one-by-one via null placeholders.

With respect to both our functional and structural metrics, we find that gene duplication can systematically increase evolved complexity.
Control treatments reveal, in particular, that genetic information in duplicated regions sparks otherwise unlikely genome expansion.
...
We thus also confirm that gene duplication can facilitate otherwise unlikely complex innovation in adaptation-driven scenarios.

Figure \ref{fig:slip_mut_variants} details our experimental design.
Digital organisms were well-mixed, with population carrying capacity of 3,600.
To supply adaptive potential, rewards were provided for Lenski \textit{et al.}'s collection of nine phenotypic traits (different logical transforms on binary inputs) \citep{Lenski2003Evolutionary}.
Under this scheme, organisms producing correct output received a metabolic boost to their genetic operations.
Formally, ``substep'' complexity of each task is described as a minimum quantity of necessary NAND operations \citep{Lenski2003Evolutionary}.
\unskip\footnote{
...
To control for the effects of genome length, we also included trials with longer 1,000-site genomes, near the upper extreme of 870-site genome length fixed under slip duplication (Supplementary Figure \ref{fig:ablation-site-counts}).
