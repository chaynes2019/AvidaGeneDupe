...
In this paper, we use direct experimentation to test how gene duplication impacts structural and functional complexity, and the extent to which such impact depends on the actual genetic information content within duplications.
...
We conducted these digital experiments using Avida, software which administers populations of self-replicating computer programs --- operating as digital ``model organisms'' \citep{Ofria:2009avida}.
Because copy errors may introduce heritable variation in how efficiently programs replicate, evolution by natural selection results \citep{pennock2007models}.
...
input/output calculations can be treated as phenotypic traits, with the complexity of a given task given according to its minimum necessary substeps.
...
To more comprehensively test complexity outcomes, we complemented such phenotypic measures of functional complexity with a measure of structural genetic complexity.
Here, we counted genome instructions contributing to an organism's fitness \citep{Adami2002}.
Adaptive sites were identified through single-site knockout screens \citep{moreno2024cryptic}, disabling genome instructions one-by-one via no-operation stand-ins.

With respect to both functional and structural metrics, we find that gene duplication can systematically increase evolved complexity.
Control treatments reveal, in particular, that genetic information in duplicated regions can spark otherwise unlikely genome expansion.
This expansion, in turn, can promote simple adaptive traits and more coding sites --- in line with neutralist perspectives on biological complexity \citep{Lynch2000,Beslon2021,Lynch2007}
However, we also find that genetic information within duplicated regions also catalyzes complex adaptive traits, beyond the effects of increased genome size alone.
Causality appears direct, as disproportionate coding sites for these complex traits trace back to slip-duplicated regions.
Gene duplication can thus also facilitate otherwise unlikely complex innovation in adaptation-driven scenarios.

Figure \ref{fig:slip_mut_variants} details our experimental design.
Digital organisms were well-mixed, with population carrying capacity up to 3,600.
To supply adaptive potential, reward was provided for Lenski \textit{et al.}'s nine defined phenotypic traits --- each a specific logical transform on binary inputs.
Under this scheme, organisms producing a correct output value receive metabolic benefit that accelerates their genetic operations.
Formally, ``substeps'' necessary to carry out each task may be quantified in terms of a minimum number of NAND operations \citep{Lenski2003Evolutionary}.
\unskip\footnote{
...
In addition to standard point mutation, our experiments
...
To control for the effects of genome length, we also included trials with longer 1,000-site genomes, which was near the upper extreme of 870-site genome length fixed under slip duplication (Supplementary Figure \ref{fig:ablation-site-counts}).
